...
Перед первым входом пользователь регистрируется с email и паролем, получает письмо и подтверждает email кнопкой из письма. После входа web подготавливает личный workspace `personal-{userId}` и проект `TASKS` для текущего пользователя. Корпоративный workspace создается с внутренним номером, а вход в него идет по этому номеру: если пользователь еще не участник, создается заявка, которую подтверждает владелец workspace.
...
Все секреты и SMTP-настройки передаются через окружение. Локальный пример находится в `.env.example`; для production нужно заменить JWT/OAuth/S3/SMTP значения на значения конкретного окружения. Если `SMTP_HOST` не задан, API не отправляет реальное письмо и пишет verification link в лог.
